...
#import "CCTHContactView.h"
#import "CCTHContactTextField.h"

@interface CCTHContactPickerView ()<THContactTextFieldDelegate>{
...
@property (nonatomic, strong) CCTHContactTextField *textField;
@property (nonatomic, strong) CCTHContactViewStyle *contactViewStyle;
@property (nonatomic, strong) CCTHContactViewStyle *contactViewSelectedStyle;

@end

@implementation CCTHContactPickerView
...
- (void)setup {
    ...
    // Create TextView
    self.textField = [[CCTHContactTextField alloc] init];
    ...
    //default settings
    CCTHContactView *contactView = [[CCTHContactView alloc] initWithName:@""];
...
- (void)setFont:(UIFont *)font {
    ...
    // Create a contact view to determine the height of a line
    CCTHContactView *contactView = [[CCTHContactView alloc] initWithName:@"Sample"];
...
- (void)addContact:(id)contact withName:(NSString *)name withStyle:(CCTHContactViewStyle *)bubbleStyle andSelectedStyle:(CCTHContactViewStyle *)selectedStyle {
    ...
    if (self.contactKeys.count == 1 && self.limitToOne){
        CCTHContactView *contactView = [self.contacts objectForKey:[self.contactKeys firstObject]];
    ...
    CCTHContactView *contactView = [[CCTHContactView alloc] initWithName:name style:bubbleStyle selectedStyle:selectedStyle showComma:_showComma];
...
- (void)removeAllContacts {
    for (id contact in [self.contacts allKeys]){
      CCTHContactView *contactView = [self.contacts objectForKey:contact];
...
- (void)setContactViewStyle:(CCTHContactViewStyle *)style selectedStyle:(CCTHContactViewStyle *)selectedStyle {
    ...
    for (id contactKey in self.contactKeys){
        CCTHContactView *contactView = (CCTHContactView *)[self.contacts objectForKey:contactKey];
...
- (void)removeContactView:(CCTHContactView *)contactView {
...
- (void)removeContactByKey:(id)contactKey {
    ...
    CCTHContactView *contactView = [self.contacts objectForKey:contactKey];
    [contactView removeFromSuperview];
...
- (id)contactForContactView:(CCTHContactView *)contactView {
...
- (void)textFieldDidHitBackspaceWithEmptyText:(CCTHContactTextField *)textView {
    self.textField.hidden = NO;
...
- (void)textFieldDidChange:(CCTHContactTextField *)textView{
    if ([self.delegate respondsToSelector:@selector(contactPickerTextViewDidChange:)]
...
- (void)setKeyboardAppearance:(UIKeyboardAppearance)keyboardAppearance {
    ...
    for (CCTHContactView *contactView in self.contacts) {
