...
#import "CCIDMZoomingScrollView.h"
...
- (CCIDMZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index;
- (CCIDMZoomingScrollView *)pageDisplayingPhoto:(id<CCIDMPhoto>)photo;
- (CCIDMZoomingScrollView *)dequeueRecycledPage;
- (void)configurePage:(CCIDMZoomingScrollView *)page forIndex:(NSUInteger)index;
...
- (CGRect)frameForCaptionView:(CCIDMCaptionView *)captionView atIndex:(NSUInteger)index;
...
- (void)loadAdjacentPhotosIfNecessary:(id<CCIDMPhoto>)photo;
- (void)releaseAllUnderlyingPhotos;

@end

// IDMPhotoBrowser
@implementation CCIDMPhotoBrowser
...
- (id)initWithPhotoURLs:(NSArray *)photoURLsArray {
    if ((self = [self init])) {
        NSArray *photosArray = [CCIDMPhoto photosWithURLs:photoURLsArray];
		_photos = [[NSMutableArray alloc] initWithArray:photosArray];
	}
	return self;
}
...
- (void)panGestureRecognized:(id)sender {
    // Initial Setup
    CCIDMZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
...
- (void)performCloseAnimationWithScrollView:(CCIDMZoomingScrollView*)scrollView {
...
- (void)viewDidLoad {
    ...
    [UIImage imageNamed:@"CCIDMPhotoBrowser.bundle/images/IDMPhotoBrowser_arrowLeft.png"]          : _leftArrowImage;
    
    UIImage *rightButtonImage = (_rightArrowImage == nil) ?
    [UIImage imageNamed:@"CCIDMPhotoBrowser.bundle/images/IDMPhotoBrowser_arrowRight.png"]         : _rightArrowImage;
    
    UIImage *leftButtonSelectedImage = (_leftArrowSelectedImage == nil) ?
    [UIImage imageNamed:@"CCIDMPhotoBrowser.bundle/images/IDMPhotoBrowser_arrowLeftSelected.png"]  : _leftArrowSelectedImage;
    
    UIImage *rightButtonSelectedImage = (_rightArrowSelectedImage == nil) ?
    [UIImage imageNamed:@"CCIDMPhotoBrowser.bundle/images/IDMPhotoBrowser_arrowRightSelected.png"] : _rightArrowSelectedImage;
...
- (void)viewWillLayoutSubviews {
	...
	for (CCIDMZoomingScrollView *page in _visiblePages) {
...
- (CCIDMCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    CCIDMCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <CCIDMPhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[CCIDMCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (void)loadAdjacentPhotosIfNecessary:(id<CCIDMPhoto>)photo {
    CCIDMZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <CCIDMPhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
...
- (void)handleIDMPhotoLoadingDidEndNotification:(NSNotification *)notification {
    id <CCIDMPhoto> photo = [notification object];
    CCIDMZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        if ([photo underlyingImage]) {
            // Successful load
...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            // Add new page
			CCIDMZoomingScrollView *page;
            page = [[CCIDMZoomingScrollView alloc] initWithPhotoBrowser:self];
            ...
            // Add caption
            CCIDMCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
...
- (CCIDMZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
	CCIDMZoomingScrollView *thePage = nil;
	for (CCIDMZoomingScrollView *page in _visiblePages) {
...
- (void)configurePage:(CCIDMZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    __block __weak CCIDMPhoto *photo = (CCIDMPhoto*)page.photo;
    __weak CCIDMZoomingScrollView* weakPage = page;
...
- (CCIDMZoomingScrollView *)dequeueRecycledPage {
	CCIDMZoomingScrollView *page = [_recycledPages anyObject];
	if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    id <CCIDMPhoto> currentPhoto = [self photoAtIndex:index];
...
- (void)actionButtonPressed:(id)sender {
    id <CCIDMPhoto> photo = [self photoAtIndex:_currentPageIndex];
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        if(!_actionButtonTitles)
        {
			...
			if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0"))
			{
				[self.activityViewController setCompletionWithItemsHandler:^(NSString *activityType, BOOL completed, NSArray *returnedItems, NSError *activityError) {
                    ...
                    if ([activityType isEqualToString:@"com.apple.UIKit.activity.SaveToCameraRoll"]) {
                        NSString *saveImageString = [[NSBundle mainBundle] localizedStringForKey:@"Saved image" value:@"" table:@"ChatCenterSDK"];
                        [selfBlock.view makeToast:saveImageString duration:1.0f position:CSToastPositionCenter];
                    }
				}];
			}
			else
            ...
            //if iPad
                ...
                [popup presentPopoverFromRect:CGRectMake(self.view.frame.size.width-50, self.view.frame.size.height - 50, 50, 50) inView:self.view permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
            }
            // [self presentViewController:self.activityViewController animated:YES completion:nil];
