...
+ (CCIDMPhoto *)photoWithImage:(UIImage *)image {
	return [[CCIDMPhoto alloc] initWithImage:image];
}

+ (CCIDMPhoto *)photoWithFilePath:(NSString *)path {
	return [[CCIDMPhoto alloc] initWithFilePath:path];
}

+ (CCIDMPhoto *)photoWithURL:(NSURL *)url {
	return [[CCIDMPhoto alloc] initWithURL:url];
}
...
+ (NSArray *)photosWithImages:(NSArray *)imagesArray {
    ...
    for (UIImage *image in imagesArray) {
        if ([image isKindOfClass:[UIImage class]]) {
            CCIDMPhoto *photo = [CCIDMPhoto photoWithImage:image];
...
+ (NSArray *)photosWithFilePaths:(NSArray *)pathsArray {
    ...
    for (NSString *path in pathsArray) {
        if ([path isKindOfClass:[NSString class]]) {
            CCIDMPhoto *photo = [CCIDMPhoto photoWithFilePath:path];
            [photos addObject:photo];
        }
    }
...
+ (NSArray *)photosWithURLs:(NSArray *)urlsArray {
    ...
    for (id url in urlsArray) {
        if ([url isKindOfClass:[NSURL class]]) {
            ...
            CCIDMPhoto *photo = [CCIDMPhoto photoWithURL:[NSURL URLWithString:url]];
            [photos addObject:photo];
        }
    }
