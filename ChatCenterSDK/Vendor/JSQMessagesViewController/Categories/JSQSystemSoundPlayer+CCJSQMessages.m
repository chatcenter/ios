...
+ (void)jsq_playSoundFromJSQMessagesBundleWithName:(NSString *)soundName asAlert:(BOOL)asAlert
{
    ...
    NSString *fileName = [NSString stringWithFormat:@"CCJSQMessagesAssets.bundle/Sounds/%@", soundName];
    
    if (asAlert) {
        [[CCJSQSystemSoundPlayer sharedPlayer] playAlertSoundWithFilename:fileName fileExtension:kJSQSystemSoundTypeAIFF];
    }
    else {
        [[CCJSQSystemSoundPlayer sharedPlayer] playSoundWithFilename:fileName fileExtension:kJSQSystemSoundTypeAIFF];
    }
