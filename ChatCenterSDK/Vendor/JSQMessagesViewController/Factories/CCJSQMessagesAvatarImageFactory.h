...
/**
 *  `CCJSQMessagesAvatarImageFactory` is a factory that provides a means for creating and styling
 *  `CCJSQMessagesAvatarImage` objects to be displayed in a `CCJSQMessagesCollectionViewCell` of a `CCJSQMessagesCollectionView`.
 */
@interface CCJSQMessagesAvatarImageFactory : NSObject

/**
*  Creates and returns a `CCJSQMessagesAvatarImage` object with the specified placeholderImage that is
...
*
*  @return An initialized `CCJSQMessagesAvatarImage` object if created successfully, `nil` otherwise.
*/
+ (CCJSQMessagesAvatarImage *)avatarImageWithPlaceholder:(UIImage *)placeholderImage diameter:(NSUInteger)diameter;

/**
 *  Creates and returns a `CCJSQMessagesAvatarImage` object with the specified image that is
 *  cropped to a circle of the given diameter and used for the `avatarImage` and `avatarPlaceholderImage` properties
 *  of the returned `CCJSQMessagesAvatarImage` object. This image is then copied and has a transparent black mask applied to it, 
 *  which is used for the `avatarHighlightedImage` property of the returned `CCJSQMessagesAvatarImage` object.
 *
...
/**
 *  Creates and returns a `CCJSQMessagesAvatarImage` object with a circular shape that displays the specified userInitials
