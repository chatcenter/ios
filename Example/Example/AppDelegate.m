...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    /// Push Notification: Register device to APNS
    /// After iOS8 and before iOS10
...
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler {
    ...
    if(application.applicationState == UIApplicationStateActive){
        NSLog(@"UIApplicationStateActive from Push");
        [UIApplication sharedApplication].applicationIconBadgeNumber = -1;
        return;
    }
    
    NSUserDefaults *ud = [NSUserDefaults standardUserDefaults];
    [ud setObject:@"YES" forKey:@"isPush"];
    [ud synchronize];
    
    if (completionHandler != nil) completionHandler(UIBackgroundFetchResultNoData);
}
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "ly.appsocial.Example" in the application's documents directory.
